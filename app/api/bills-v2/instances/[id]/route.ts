...
import { toLegacyInstance } from '@/lib/bills-v2/legacy-compat';
...
export async function GET(
  ...
  try {
    ...
    return Response.json(toLegacyInstance(occurrence));
  } catch (error) {
    return toBillsV2Error(error, 'compat instance [id] GET');
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (payload.lateFee !== undefined) next.lateFeeCents = Math.max(0, Math.round(payload.lateFee * 100));
    ...
    if (payload.status) {
      if (payload.status === 'skipped') {
        ...
        const providedCents =
          payload.actualAmount !== undefined && payload.actualAmount !== null
            ? Math.round(payload.actualAmount * 100)
      ...
      if (payload.actualAmount !== undefined) {
        const actualCents = payload.actualAmount === null ? null : Math.max(0, Math.round(payload.actualAmount * 100));
    ...
    return Response.json(toLegacyInstance(updated));
  } catch (error) {
    return toBillsV2Error(error, 'compat instance [id] PUT');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return toBillsV2Error(error, 'compat instance [id] DELETE');
  }
}
