...
export async function GET(request: Request) {
  try {
    ...
    if (!bestOccurrence || !bestTemplate) {
      return Response.json({
        suggestedBillId: linkedTemplates[0].id,
        suggestedInstanceId: null,
        detectedBill: null,
        confidence: 'low',
    ...
    const status: 'pending' | 'overdue' | 'partial' =
    ...
    return Response.json({
      suggestedBillId: template.id,
      suggestedInstanceId: occurrence.id,
      detectedBill: {
        billId: template.id,
    ...
    return toBillsV2Error(error, 'compat detect-payment GET');
  }
}
