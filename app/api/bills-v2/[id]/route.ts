...
import {
  buildUpdateTemplateRequestFromLegacy,
...
export async function GET(
  ...
  try {
    ...
    const bill = toLegacyBill(
      {
        ...template,
        debtInterestType: template.debtInterestType || null,
      },
      toAutopayConfig(autopayRule)
    );
    ...
    return toBillsV2Error(error, 'compat [id] GET');
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const { template: updateInput, autopay } = buildUpdateTemplateRequestFromLegacy(payload);

    const updated = await updateBillTemplate(id, householdId, updateInput);

    if (autopay !== undefined) {
      await upsertAutopayRule(id, householdId, autopay);
    }

    const autopayRow = await db
      .select()
      .from(autopayRules)
      .where(and(eq(autopayRules.templateId, id), eq(autopayRules.householdId, householdId)))
      .limit(1)
    ...
    return Response.json({
      bill: toLegacyBill(updated, toAutopayConfig(autopayRow)),
      success: true,
    });
  } catch (error) {
    return toBillsV2Error(error, 'compat [id] PUT');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return toBillsV2Error(error, 'compat [id] DELETE');
  }
}
