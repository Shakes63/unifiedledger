      ...
      <div className="max-w-7xl mx-auto space-y-8">
              ...
              <CardContent>
                <div className="space-y-4">
                  {Array.from(
                    ...
                    return (
                      <div key={formType} className={`border-l-2 pl-4 ${isBusinessForm ? 'border-[var(--color-primary)]' : 'border-[var(--color-success)]'}`}>
                        <div className="flex items-center gap-2">
                          <p className="text-foreground font-semibold">
                            {formType.replace(/_/g, ' ').toUpperCase()}
                          </p>
                          {isBusinessForm ? (
                            <span className="text-xs text-[var(--color-primary)]">(Business)</span>
                          ) : formType === 'schedule_a' ? (
                            <span className="text-xs text-[var(--color-success)]">(Personal)</span>
                          ) : null}
                        </div>
                        <p className={`text-2xl font-bold ${isBusinessForm ? 'text-[var(--color-primary)]' : 'text-[var(--color-success)]'}`}>
                          ${total.toFixed(2)}
                        </p>
        ...
        {hasData && (
          <Card className="bg-[var(--color-warning)]/10 border-[var(--color-warning)]/30">
            <CardHeader>
              <CardTitle className="text-[var(--color-warning)] flex items-center gap-2">
