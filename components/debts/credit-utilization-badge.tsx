...
export function CreditUtilizationBadge({
  ...
  return (
    <div className="inline-flex items-center gap-1">
      {showLabel && (
        <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>Utilization:</span>
      )}
      ...
      <div className="relative group">
        ...
        {showTooltip && (
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none">
            <div className="rounded-lg p-3 shadow-lg min-w-[200px]" style={{ backgroundColor: 'var(--color-background)', border: '1px solid var(--color-border)' }}>
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>Balance:</span>
                  <span className="text-sm font-mono" style={{ color: 'var(--color-foreground)' }}>
                    ${balance.toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>Limit:</span>
                  <span className="text-sm font-mono" style={{ color: 'var(--color-foreground)' }}>
                    ${creditLimit.toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>Available:</span>
              ...
              {/* Tooltip arrow */}
              <div
                className="absolute top-full left-1/2 -translate-x-1/2 -mt-px w-2 h-2 border-r border-b rotate-45"
                style={{ backgroundColor: 'var(--color-background)', borderRight: '1px solid var(--color-border)', borderBottom: '1px solid var(--color-border)' }}
              />
            </div>
          </div>
        )}
