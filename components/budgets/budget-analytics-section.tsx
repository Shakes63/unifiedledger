...
export function BudgetAnalyticsSection({ hideHeader = false }: BudgetAnalyticsSectionProps) {
  ...
  const getSavingsRateLabel = (rate: number) => {
    if (rate >= 20) return { label: 'Excellent', color: 'text-success' };
    if (rate >= 10) return { label: 'Good', color: 'text-success' };
    if (rate >= 5) return { label: 'Fair', color: 'text-warning' };
  ...
  const getPriorityColor = (priority: string) => {
    if (priority === 'high') return 'border-error bg-error/10';
    if (priority === 'medium') return 'border-warning bg-warning/10';
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          <div className="text-error">{error || 'Failed to load analytics'}</div>
          <button
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          ...
          <div className={`text-2xl font-semibold ${analyticsData.summary.averageMonthlySavings >= 0 ? 'text-success' : 'text-error'}`}>
      ...
      {analyticsData.overspendingRanking.length > 0 && (
        <div className="bg-card border border-warning rounded-xl p-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-2">
                {analyticsData.overspendingRanking.slice(0, 3).map(cat => (
                  <span
                    key={cat.categoryId}
                    className="text-xs px-3 py-1.5 bg-warning/10 text-warning rounded-md font-medium"
                  >
                    {cat.categoryName}
                  </span>
                ))}
