...
- ~~**Bill Instance Operations Not Implemented** - The bill detail page (`/dashboard/bills/[id]`) displays bill instances (overdue and upcoming) but does not provide UI for managing them. Missing features: "Mark as Paid" button, "Mark as Pending" button, "Link to Transaction" functionality. Users cannot manually track bill payments from the bill detail view. Bill auto-detection via transactions works, but manual status updates are not available.~~ **FIXED 2025-12-06**: Full `BillInstanceActionsModal` component with "Mark as Paid", "Skip", "Mark as Pending" buttons, "Link Transaction" tab with smart match scoring, and actual amount/notes fields. UI is accessible via dropdown menu on each bill instance.
