...
- [ ] **Offline Sync Queue** - Add automated coverage for offline transaction sync orchestration + queuing (sync, retries, status, discard). **Status:** In progress (✅ added `lib/offline/offline-sync.ts` unit tests). **Remaining:** hook coverage (`useOfflineTransaction`) and optional IndexedDB queue tests. **Plan:** `docs/offline-sync-queue-tests-plan.md`
