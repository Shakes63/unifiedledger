...
- [ ] **Offline Sync Queue** - Add automated coverage for offline transaction sync orchestration + queuing (sync, retries, status, discard). **Status:** In progress (✅ added `lib/offline/offline-sync.ts` unit tests, ✅ added `hooks/useOfflineTransaction.ts` hook tests). **Remaining:** optional IndexedDB-level queue tests. **Plan:** `docs/offline-sync-queue-tests-plan.md`
- [ ] **Admin User Management (Test Coverage)** - Add automated coverage for `/api/admin/users` (CRUD, search/pagination), owner gating, validation/error cases. **Status:** In progress (✅ added initial API tests for GET/POST). **Remaining:** tests for `PUT/DELETE /api/admin/users/[userId]` + additional search/pagination edge cases. **Plan:** `docs/admin-user-management-tests-plan.md`
