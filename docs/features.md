...
- [ ] **Two-Factor Authentication (2FA) (Test Coverage)** - Add automated tests for 2FA API routes + core utilities. **Status:** In progress (✅ added initial API tests for status + enable). **Remaining:** verify/disable/backup-codes/verify-login endpoints + `lib/auth/two-factor-utils.ts` unit tests. **Plan:** `docs/two-factor-auth-test-coverage-plan.md`
