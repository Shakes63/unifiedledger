...
export const bills = pgTable(
  'bills',
  {
    ...
    amountTolerance: doublePrecision('amount_tolerance').default(5.0),
    payeePatterns: text('payee_patterns'),
    accountId: text('account_id'),
    isActive: boolean('is_active').default(true),
    autoMarkPaid: boolean('auto_mark_paid').default(true),
    notes: text('notes'),
    ...
    // Autopay settings
    isAutopayEnabled: boolean('is_autopay_enabled').default(false),
    autopayAccountId: text('autopay_account_id'),
    autopayAmountType: text('autopay_amount_type', {
      enum: ['fixed', 'minimum_payment', 'statement_balance', 'full_balance'],
    }),
    autopayFixedAmount: doublePrecision('autopay_fixed_amount'),
    ...
    originalBalance: doublePrecision('original_balance'),
    ...
    isInterestTaxDeductible: boolean('is_interest_tax_deductible').default(false),
    taxDeductionType: text('tax_deduction_type', {
      enum: ['mortgage', 'student_loan', 'business', 'heloc_home', 'none'],
    }).default('none'),
    taxDeductionLimit: doublePrecision('tax_deduction_limit'),
...
export const billInstances = pgTable(
  'bill_instances',
  {
    ...
    principalPaid: doublePrecision('principal_paid').default(0),
    interestPaid: doublePrecision('interest_paid').default(0),
